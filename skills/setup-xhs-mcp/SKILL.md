...
判断：
- 返回 `405 Method Not Allowed` / `GET requires an active session` → 服务已运行，记录地址 `http://localhost:18060/mcp`，跳到步骤 3
- `Failed to connect to localhost port 18060` → 服务未运行，先尝试本机已存在仓库的 Docker Compose 重启：
...
本机实操路径：`/home/bee/apps/xiaohongshu-mcp/docker/docker-compose.yml`，容器名通常是 `xiaohongshu-mcp`。

- 重启成功 → 记录地址 `http://localhost:18060/mcp`，跳到步骤 3
- 仍不可达 → 询问用户：服务是否部署在其他地址/端口？
  - 用户提供地址 → 验证可达后跳到步骤 3
  - 未部署 → 进入步骤 2
